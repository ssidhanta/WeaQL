...
public class EB extends Thread {
  public String webproxy; //web proxy base url (http://address:port)
  // Terminate all EBs.
  ...
  void  getHTML(URL url) throws IOException
  {
   int nretry=0;
   if(DEBUG>0)  System.out.println("Begin reading HTML. " + name);
    html = "";
    int r;
	...
	retry=true;
	nretry++;
	if(nretry>=rbe.userMaxRetry)	throw ioe;
	continue;
      }
      try {
