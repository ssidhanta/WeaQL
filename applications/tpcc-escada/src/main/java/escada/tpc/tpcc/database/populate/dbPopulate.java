			...
			for (int i = 1; i < (numWareh + 1); i++) {
				for (int j = 1; j < (TPCCConst.getNumDistrict() + 1); j++) {
					for (int a = 1; a < (TPCCConst.getNumCustomer() + 1); a++) {
						...
						pstmt.setInt(8, 1);
						b++;
						//FIXME: the corrected line. orders have a FK to customer, if we hardcoded the value '3000'
						//if we change the variable numberCustomers it will not be reflected here
						if (b > (TPCCConst.getNumCustomer() - 1)) {
						//if (b > 3000) {
							b = 1;
						}
						pstmt.executeUpdate();
