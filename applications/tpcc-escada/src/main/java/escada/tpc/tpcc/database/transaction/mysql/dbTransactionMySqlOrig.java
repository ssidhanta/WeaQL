 ...
 */
public class dbTransactionMySqlOrig extends dbTPCCDatabase
{

	private Logger logger = Logger.getLogger(dbTransactionMySqlOrig.class);
	...
	protected HashSet NewOrderDB(Properties obj, Connection con) throws SQLException
	{
		...
		while(true)
		{
			...
			try
			{
				...
				statement = con.prepareStatement(
						"select c_last, c_discount, c_credit, c_id from customer where c_id = ? and c_w_id = ? and " +
								"c_d_id = ?");
				...
				while(_li_no < _o_ol_cnt)
				{
					...
					_li_s_w_id = Integer.parseInt((String) obj.get("supwid" + _li_no));
					_li_qty = Integer.parseInt((String) obj.get("qtdi" + _li_no));
					...
					statement = con.prepareStatement(
							"update stock set s_ytd = s_ytd + " + _li_qty + ", s_order_cnt = s_order_cnt + 1 where " +
									"s_i_id = ? and s_w_id = ?");
					statement.setInt(1, _li_id);
					...
					statement = con.prepareStatement(
							"update stock set s_quantity = ?, s_remote_cnt = ? where s_i_id = ? and s_w_id = ?");
			...
			} catch(SQLException sqlex)
			{
				...
				if((sqlex.getMessage().indexOf("serialize") != -1) || (sqlex.getMessage().indexOf(
						"timeout") != -1) || (sqlex.getMessage().indexOf("Deadlock") != -1))
				{
		...
		while(true)
		{
			...
			try
			{
				...
				while(_d_id < 10)
				{
					...
					_o_id = 0;
						 /*
					String sql = "select no_o_id from new_orders where no_w_id = ? and no_d_id = ? order by no_o_id " +
							"asc limit 1";
					statement = con.prepareStatement(sql);
					    */

					statement = con.prepareStatement("select no_o_id from new_orders where no_w_id = ? and no_d_id = " +
					...
					if(_o_id != 0)
					{
						/*
						statement = con.prepareStatement(
								"delete from new_orders where no_w_id = ? and no_d_id = ? and no_o_id = ?");
						statement.setInt(1, _w_id);
						statement.setInt(2, _d_id);
						...
						*/
						...
						statement = con.prepareStatement(
								"update order_line set ol_delivery_d = CURRENT_TIMESTAMP where ol_w_id = ? " + "and " +
										"ol_d_id = ? and ol_o_id = ?");
						statement.setInt(1, _w_id);
						statement.setInt(2, _d_id);
						statement.setInt(3, _o_id);
						...
						statement = con.prepareStatement(
								" select ol_amount from order_line where ol_w_id = ? and  ol_d_id = ? " + " and " +
										"ol_o_id = ?");
						statement.setInt(1, _w_id);
						statement.setInt(2, _d_id);
						statement.setInt(3, _o_id);
						...
						statement = con.prepareStatement(
								"update customer set c_balance = c_balance + " + _total + "," + " c_delivery_cnt = " +
	...
	protected HashSet OrderStatusDB(Properties obj, Connection con) throws SQLException
	{
		...
		while(true)
		{
			...
			try
			{
				...
				{
					statement = con.prepareStatement(
							"select c_id, c_last, c_balance, c_first, c_middle " + " from customer where c_last = ? " +
									"and " + " c_w_id = ? and c_d_id = ? " + " order by c_w_id, c_d_id, c_last, " +
									"c_first limit 1 ");
				...
				{
					statement = con.prepareStatement(
							"select c_id, c_last, c_balance, c_first, c_middle " + " from customer where c_id = ? and " +
									"c_d_id = ? and c_w_id = ?");
				...
				{
					statement = con.prepareStatement(
							"select o_id, o_entry_d, o_carrier_id from orders where o_c_id = ? " + " and o_d_id = ? " +
									"and o_w_id = ? order by o_id asc limit 1");
					statement.setInt(1, _c_id);
					statement.setInt(2, __d_id);
					statement.setInt(3, __w_id);
					rs = statement.executeQuery();
					...
					if(rs.next())
					{
						...
						statement = con.prepareStatement(" select * from order_line where ol_o_id = ? and  ol_d_id = ? " + " and ol_w_id = ?");
						statement.setInt(1, _o_id);
						statement.setInt(2, __d_id);
						statement.setInt(3, __w_id);
						rs = statement.executeQuery();
				...
				if(str.equals("0"))
				{
					processLog(NetStartTime, NetFinishTime, "processing", "r", "tx orderstatus 01");
				} else
				{
	...
	protected HashSet PaymentDB(Properties obj, Connection con) throws SQLException
	{
		...
		while(true)
		{
			...
			try
			{
				...
				float __h_amount = Float.parseFloat((String) obj.get("hamount"));
				int __d_id = Integer.parseInt((String) obj.get("did"));
				...
				{
					statement = con.prepareStatement(
							"select c_id from customer where c_last = ? and " + " c_w_id = ? and c_d_id = ? order by " +
									"c_w_id, c_d_id, c_last, c_first limit 1");
					statement.setString(1, __c_last);
				...
				statement = con.prepareStatement(
						"update customer set c_balance = c_balance - " + __h_amount + "," + " c_payment_cnt = " +
								"c_payment_cnt + 1, c_ytd_payment = c_ytd_payment + " + __h_amount + "  where c_id = ?" +
								" and c_w_id = ? and  c_d_id = ?");
				statement.setInt(1, _c_id);
				statement.setInt(2, __c_w_id);
				statement.setInt(3, __c_d_id);
				statement.executeUpdate();
				...
				if(_c_credit.equals("BC"))
				{
					...
					statement = con.prepareStatement(
							"update customer set c_data = ? where c_id = ? and " + " c_w_id = ? and c_d_id = ?");
				...
				statement = con.prepareStatement(
						"update district set d_ytd = d_ytd + " + __h_amount + " where d_w_id = ? and d_id = ? ");
				...
				statement = con.prepareStatement(
						"update `warehouse`  set `w_ytd` = `w_ytd` + " + __h_amount + "  where `w_id` = ?");
				statement.setInt(1, __w_id);
				statement.executeUpdate();
				statement.close();
				...
				if(str.equals("0"))
				{
					processLog(NetStartTime, NetFinishTime, "processing", "w", "tx payment 01");
				} else
				{
	...
	protected HashSet StockLevelDB(Properties obj, Connection con) throws SQLException
	{
		...
		while(true)
		{
			...
			try
			{
				...
				statement = con.prepareStatement(
						"select count(distinct(s_i_id)) from stock, order_line where ol_w_id = ? and " + " ol_d_id = ?" +
								" and ol_o_id >= ? and ol_o_id <= ? and s_w_id = ol_w_id and s_i_id = ol_i_id and " +
								"s_quantity < ? ");
				statement.setInt(1, __w_id);
				statement.setInt(2, __d_id);
