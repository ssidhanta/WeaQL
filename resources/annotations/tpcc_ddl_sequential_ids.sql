@UOSETTABLE @AW create table warehouse (
	...
	@NORMALSTRING status varchar(24),
	PRIMARY KEY (w_id) 
)Engine=InnoDB;

@UOSETTABLE @AW  CREATE TABLE cnflct_flgs (
    c_tbl_chld varchar(20),
    c_tbl_chld_pk varchar(20),
    c_tbl_flgs varchar(20)
) ENGINE=InnoDB;

@UOSETTABLE @RW create table district (
...
@UOSETTABLE @AW create table customer (
...
@AOSETTABLE @AW create table history (
	...
	@NORMALSTRING status varchar(24),
	PRIMARY KEY(h_id),
	FOREIGN KEY(h_w_id,h_d_id) @FR REFERENCES district(d_w_id,d_id)
)Engine=InnoDB;

@AOSETTABLE @RW create table orders (
...
@ARSETTABLE @AW create table new_orders (
	...
	FOREIGN KEY(no_w_id,no_d_id,no_o_id) @FR REFERENCES orders(o_w_id,o_d_id,o_id)	
)Engine=InnoDB;

@READ_ONLY_TABLE @AW create table item (
...
@UOSETTABLE @RW create table stock (
	...
	FOREIGN KEY(s_w_id) @FR REFERENCES warehouse(w_id),
	FOREIGN KEY(s_i_id) @FR REFERENCES item(i_id)
)Engine=InnoDB;

@AUSETTABLE @RW create table order_line (
